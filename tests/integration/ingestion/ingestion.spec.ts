  ...
  describe('PUT /ingestion/:jobId/retry', () => {
    ...
    describe('Sad Path', () => {
      ...
      it('should return 400 BAD_REQUEST status code when validation is invalid and input files have invalid schema', async () => {
        ...
        expect(response.status).toBe(httpStatusCodes.BAD_REQUEST);
        expect(response.body).toHaveProperty('message');
        expect((response.body as { message: string }).message).toContain('gpkgFilesPath: Files should be an array of .gpkg file names | metadataShapefilePath: Required | productShapefilePath: Required');
      });
