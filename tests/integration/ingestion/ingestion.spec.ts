...
describe('Ingestion', () => {
  ...
  describe('PUT /ingestion/:jobId/retry', () => {
    describe('Happy Path', () => {
      it('should return 200 status code when retrying a failed job with valid validation (no errors)', async () => {
        ...
        nock(jobManagerURL).get(`/jobs/${jobId}`).reply(httpStatusCodes.OK, mockJob);
        nock(jobManagerURL).get(`/jobs/${jobId}/tasks`).reply(httpStatusCodes.OK, [mockValidationTask]);
        ...
        const response = await requestSender.retryLayer(jobId);
      ...
      it('should return 200 status code when retrying a completed job with shapefile changes', async () => {
        const jobId = faker.string.uuid();
        const taskId = faker.string.uuid();
        const existingChecksum = { fileName: 'ShapeMetadata.shp', checksum: 'oldChecksum123' };
        const mockJob = {
          id: jobId,
          status: OperationStatus.COMPLETED,
          parameters: {
            inputFiles: {
              gpkgFilesPath: [validInputFiles.inputFiles.gpkgFilesPath[0]],
              metadataShapefilePath: validInputFiles.inputFiles.metadataShapefilePath,
              productShapefilePath: validInputFiles.inputFiles.productShapefilePath,
            },
          },
        };
        const mockValidationTask = {
    ...
    describe('Bad Path', () => {
      it('should return 400 status code when job ID is not a valid UUID', async () => {
        const invalidJobId = 'not-a-uuid';

        const response = await requestSender.retryLayer(invalidJobId);

        expect(response).toSatisfyApiSpec();
        expect(response.status).toBe(httpStatusCodes.BAD_REQUEST);
      });

      it('should return 400 status code when job status is PENDING', async () => {
        const jobId = faker.string.uuid();
        const mockJob = {
          id: jobId,
      ...
      it('should return 400 status code when job status is IN_PROGRESS', async () => {
        const jobId = faker.string.uuid();
        const mockJob = {
          id: jobId,
      ...
      it('should return 400 status code when metadataShapefilePath is missing from job parameters', async () => {
        const jobId = faker.string.uuid();
        const taskId = faker.string.uuid();
        const mockJob = {
          id: jobId,
        ...
        const mockValidationTask = {
          ...
          parameters: {
            isValid: false,
            checksums: [{ fileName: 'ShapeMetadata.shp', checksum: 'checksum123' }],
          },
        };
    ...
    describe('Sad Path', () => {
      it('should return 404 status code when job is not found', async () => {
        const jobId = faker.string.uuid();

        nock(jobManagerURL).get(`/jobs/${jobId}`).reply(httpStatusCodes.NOT_FOUND);
      ...
      it('should return 404 status code when validation task is not found', async () => {
        const jobId = faker.string.uuid();
        const mockJob = {
          id: jobId,
          status: OperationStatus.FAILED,
      ...
      it('should return 409 status code when shapefile has not changed', async () => {
        const jobId = faker.string.uuid();
        const taskId = faker.string.uuid();
        const mockJob = {
          id: jobId,
      ...
      it('should return 500 status code when job manager returns unexpected error', async () => {
        const jobId = faker.string.uuid();
      ...
      it('should return 500 status code when failed to reset job', async () => {
        const jobId = faker.string.uuid();
        const taskId = faker.string.uuid();
        const mockJob = {
          id: jobId,
        ...
        nock(jobManagerURL).get(`/jobs/${jobId}`).reply(httpStatusCodes.OK, mockJob);
        nock(jobManagerURL).get(`/jobs/${jobId}/tasks`).reply(httpStatusCodes.OK, [mockValidationTask]);
        nock(jobManagerURL).post(`/jobs/${jobId}/reset`).reply(httpStatusCodes.INTERNAL_SERVER_ERROR);
      ...
      it('should return 500 status code when failed to update validation task', async () => {
        const jobId = faker.string.uuid();
        const taskId = faker.string.uuid();
        const existingChecksum = { fileName: 'ShapeMetadata.shp', checksum: 'oldChecksum123' };
        const mockJob = {
